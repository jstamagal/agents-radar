...
export async function saveSignalsReport(
  ...
  console.log(`  [signals/${lang}] Calling LLM for signals panorama report...`);
  try {
    const signalsSummary = await callLlm(buildSignalsPrompt(trendingData, hnData, webResults, dateStr, lang));
