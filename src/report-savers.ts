...
// ---------------------------------------------------------------------------
// Signals radar report — wide-view consolidated table of all trending signals
// ---------------------------------------------------------------------------
...
export async function saveSignalsReport(
  ...
  console.log(`  [signals/${lang}] Calling LLM for signals radar report...`);
  try {
    const signalsSummary = await callLlm(buildSignalsPrompt(trendingData, dateStr, lang));
